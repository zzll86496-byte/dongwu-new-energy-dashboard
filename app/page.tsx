...
function TrendChart({ lines, unit, height = 270 }: { lines: ChartLine[]; unit: string; height?: number }) {
  const [hoverIndex, setHoverIndex] = useState<number | null>(null);
  const labels = lines[0]?.points.map((point) => point[0]) ?? [];
...
function OverviewMetricChart({ title, points, color }: { title: string; points: Point[]; color: string }) {
  ...
  return <div className="overview-chart-card">
    ...
    <TrendChart unit={mode === "total" ? "GWh" : "%"} height={300} lines={[{ name: mode === "total" ? "全球总量" : "全球环比", color, points: mode === "total" ? points : momPoints }]} />
  </div>;
}
...
function Ranking({ entities, selected, onSelect, unit, percent = false }: { entities: Entity[]; selected: string; onSelect: (name: string) => void; unit: string; percent?: boolean }) {
  ...
  return <div className="ranking">{ranked.map((entity, index) => <button key={entity.name} className={selected === entity.name ? "selected" : ""} onClick={() => onSelect(entity.name)}><em>{String(index + 1).padStart(2, "0")}</em><span>{entity.name}</span><i><b style={{ width: `${Math.max(2, entity.latest / max * 100)}%` }} /></i><strong>{fmt(entity.latest, percent ? 1 : 2)}{percent ? "%" : ""}</strong></button>)}</div>;
}
...
export default function Page() {
  ...
  const renderDetail = () => {
    if (active === "价格") return <>
      <section className="panel primary-panel"><PanelTitle index="01" title={`${selectedPrice.name}价格趋势`} sub="最低 / 均价 / 最高 · 鼠标移动查看每期数据" /><TrendChart unit="元/Wh" lines={[
    ...
    if (active === "产量") return <>
      <section className="panel primary-panel"><PanelTitle index="01" title="全球储能电池产量" sub={`叠加查看：${selectedProduction.name}`} /><TrendChart unit="GWh" lines={[{ name: "全球合计", color: COLORS[0], points: data.production.total }, { name: selectedProduction.name, color: COLORS[1], points: selectedProduction.values }]} /></section>
      <section className="panel side-panel"><PanelTitle index="02" title="企业产量排行" sub="最新月 · 点击联动" /><Ranking entities={data.production.companies} selected={productionName} onSelect={setProductionName} unit="GWh" /></section>
    ...
    if (active === "产能") return <>
      <section className="panel primary-panel"><PanelTitle index="01" title="全球储能电池产能" sub={`叠加查看：${selectedCapacity.name}`} /><TrendChart unit="GWh/年" lines={[{ name: "全球合计", color: COLORS[0], points: data.capacity.total }, { name: selectedCapacity.name, color: COLORS[2], points: selectedCapacity.values }]} /></section>
      <section className="panel side-panel"><PanelTitle index="02" title="企业产能排行" sub="最新月 · 点击联动" /><Ranking entities={data.capacity.companies} selected={capacityName} onSelect={setCapacityName} unit="GWh/年" /></section>
    ...
    if (active === "开工率") return <>
      <section className="panel primary-panel"><PanelTitle index="01" title="中国储能电池开工率" sub={`行业平均与 ${selectedUtilization.name}`} /><TrendChart unit="%" lines={[{ name: "行业平均", color: COLORS[0], points: data.utilization.average.map((p) => [p[0], p[1] === null ? null : p[1] * 100]) }, { name: selectedUtilization.name, color: COLORS[3], points: selectedUtilization.values }]} /></section>
      <section className="panel side-panel"><PanelTitle index="02" title="企业开工率排行" sub="最新月 · 点击联动" /><Ranking entities={data.utilization.companies} selected={utilizationName} onSelect={setUtilizationName} unit="%" percent /></section>
    ...
    if (active === "出货量") return <>
      <section className="panel primary-panel"><PanelTitle index="01" title="全球储能电池出货量" sub={`行业合计与 ${selectedShipment.name}`} /><TrendChart unit="GWh" lines={[{ name: "全球合计", color: COLORS[0], points: data.shipments.total }, { name: selectedShipment.name, color: COLORS[1], points: selectedShipment.values }]} /></section>
      <section className="panel side-panel"><PanelTitle index="02" title="企业出货排行" sub="最新月 · 点击联动" /><Ranking entities={data.shipments.companies} selected={shipmentName} onSelect={setShipmentName} unit="GWh" /></section>
    ...
    return <>
      ...
      <section className="panel coverage-panel"><PanelTitle index="02" title="数据覆盖" sub="三个Sheet已全部接入" /><dl><div><dt>价格品类</dt><dd>11 类</dd></div><div><dt>价格历史记录</dt><dd>{data.price.categories.reduce((n, item) => n + item.series.length, 0).toLocaleString("zh-CN")} 条</dd></div><div><dt>产量企业</dt><dd>{data.production.companies.length} 家</dd></div><div><dt>产能企业</dt><dd>{data.capacity.companies.length} 家</dd></div><div><dt>开工率企业</dt><dd>{data.utilization.companies.length} 家</dd></div><div><dt>出货量企业</dt><dd>{data.shipments.companies.length} 家</dd></div></dl></section>
    </>;
  };
