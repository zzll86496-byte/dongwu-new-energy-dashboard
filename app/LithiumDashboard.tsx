...
export function LithiumDashboard() {
  ...
  const selectedMomMax = Math.max(...selectedCompany.values.filter((value) => periods.includes(value.period)).map((value) => Math.abs(value.mom ?? 0)), .01);
  ...
  return <main className="dw-shell">
    ...
    <section className="dw-workspace">
      ...
      <div className="dw-grid">
        ...
        <section className="dw-card dw-company"><div className="dw-card-head"><b>{selectedCompany.name}{mode === "value" ? "纵向产量" : "纵向环比"}</b><span>{mode === "value" ? category.unit : "%"}</span></div><div className="dw-company-chart"><div className="dw-y-axis"><span>{mode === "value" ? fmt(selectedMax, 0) : `${(selectedMomMax * 100).toFixed(1)}%`}</span><span>{mode === "value" ? fmt(selectedMax / 2, 0) : `${(selectedMomMax * 50).toFixed(1)}%`}</span><span>0</span></div><div className="dw-bars">{selectedCompany.values.filter((value) => periods.includes(value.period)).map((value) => { const chartValue = mode === "value" ? (value.value ?? 0) / selectedMax : Math.abs(value.mom ?? 0) / selectedMomMax; const isNegative = (value.mom ?? 0) < 0; return <button className={value.period === period ? "active" : ""} key={value.period} onClick={() => setPeriod(value.period)}><span className={mode === "mom" ? (isNegative ? "mom-negative" : "mom-positive") : ""}>{mode === "value" ? fmt(value.value) : signed(value.mom)}</span><i className={mode === "value" ? "gray" : (isNegative ? "mom-negative" : "mom-positive")} style={{ height: `${Math.max(chartValue * 100, 3)}%` }} /><small>{shortPeriod(value.period)}</small></button>; })}</div></div></section>
