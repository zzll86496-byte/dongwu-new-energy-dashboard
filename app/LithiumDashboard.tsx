...
const momAt = (company: Company, period: string) => {
  const index = company.values.findIndex((value) => value.period === period);
  if (index <= 0) return null;
  const current = company.values[index].value;
  const previous = company.values[index - 1].value;
...
export function LithiumDashboard() {
  ...
  const selectedMomMax = Math.max(...selectedCompany.values.filter((value) => periods.includes(value.period)).map((value) => Math.abs(momAt(selectedCompany, value.period) ?? 0)), .01);
